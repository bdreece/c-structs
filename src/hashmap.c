...
int hashmap_init(hashmap_t *map, const size_t key_size, const size_t val_size,
  ...
  for (i = 0; i < initial_breadth; i++) {
    ...
    if ((ret = vla_enq(&map->vla, (const void *)&m)) < 0)
      return ret;
  }
...
int hashmap_deinit(hashmap_t *map) {
  ...
  for (i = 0; i < n; i++) {
    // Get map
    if ((ret = vla_get(&map->vla, i, (void *)&m)) < 0)
      return ret;

    // Deinitialize map
    if ((ret = map_deinit(m)) < 0)
      return ret;
...
int hashmap_get(const hashmap_t *map, const void *key, void *val) {
  ...
  if ((ret = vla_get(&map->vla, index, (void *)&m)) < 0)
    return ret;

  // Get value from map
  if ((ret = map_get(m, key, val)) < 0)
    return ret;
...
int hashmap_keys(const hashmap_t *map, vla_t *keys) {
  ...
  for (i = 0; i < vla_size(&map->vla); i++) {
    ...
    // Get map's keys
    if ((ret = map_keys(m, &tmp)) < 0)
      return ret;

    // Extend keys VLA
    if ((ret = vla_ext(keys, &tmp)) < 0)
      return ret;

    // Clear temp VLA
    if ((ret = vla_clear(&tmp)) < 0)
      return ret;
  }
...
int hashmap_vals(const hashmap_t *map, vla_t *vals) {
  ...
  for (i = 0; i < vla_size(&map->vla); i++) {
    ...
    // Get map's vals
    if ((ret = map_vals(m, &tmp)) < 0)
      return ret;

    // Extend vals VLA
    if ((ret = vla_ext(vals, &tmp)) < 0)
      return ret;
...
int hashmap_pairs(const hashmap_t *map, vla_t *pairs) {
  ...
  // Initialize temp VLA
  if ((ret = vla_init(&tmp, sizeof(pair_t), pairs->capacity)) < 0)
    return ret;
  ...
  for (i = 0; i < vla_size(&map->vla); i++) {
    ...
    // Get map's pairs
    if ((ret = map_pairs(m, &tmp)) < 0)
      return ret;

    // Extend pairs VLA
    if ((ret = vla_ext(pairs, &tmp)) < 0)
      return ret;
