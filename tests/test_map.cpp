...
struct umap_populated_fixture {
    umap_populated_fixture() {
        ...
        for (int i = 0; i < 5; i++) {
            BOOST_TEST_REQUIRE(map_ins(&map, (const void *)&keys[i],
                                       (const void *)&vals[i]) == ERR_NONE);
        }

        BOOST_TEST_REQUIRE(map_length(&map) == 5);
    }
...
static void test_empty_get(map_t *map) {
    ...
    BOOST_TEST(map_get(map, (const void *)&key, (void *)&val) == ERR_NOT_FOUND);
}
...
static void test_empty_getp(map_t *map) {
    ...
    BOOST_TEST(map_getp(map, (const void *)&key, (void **)&val) ==
               ERR_NOT_FOUND);
}
...
static void test_empty_set(map_t *map) {
    char key = 'a';
    int val = 1;

    BOOST_TEST(map_set(map, (void *)&key, (void *)&val) == ERR_NOT_FOUND);
}
...
BOOST_AUTO_TEST_CASE(empty_set) { test_empty_set(&map); }
...
static void test_ins_five_elements(map_t *map) {
    char keys[5] = {'a', 'b', 'c', 'd', 'e'};
    int vals[5] = {1, 2, 3, 4, 5};

    for (int i = 0; i < 5; i++) {
        BOOST_TEST(map_ins(map, (void *)&keys[i], (void *)&vals[i]) ==
                   ERR_NONE);
    }
...
BOOST_FIXTURE_TEST_CASE(umap_ins_five_elements, umap_empty_fixture) {
    test_ins_five_elements(&map);
}

BOOST_FIXTURE_TEST_CASE(omap_ins_five_elements, omap_empty_fixture) {
    test_ins_five_elements(&map);
}
...
static void test_getp_five_elements(map_t *map) {
    char keys[5] = {'a', 'b', 'c', 'd', 'e'};
    int *vals[5] = {NULL, NULL, NULL, NULL, NULL};
